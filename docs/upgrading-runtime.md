...
- `SANDBOX_SCOPE_BACKENDS` optionally maps scope kinds to their default providers,
  for example `{"personal":"modal","channel":"e2b"}`. Unlisted kinds use
  `SANDBOX_BACKEND`. Explicit per-scope routes and selected sandbox resources
  retain precedence. Configure credentials for every selected backend in the
  chart's `secretEnv`. Use distinct provider app/name prefixes for deployments
