...
function sandboxBackendEnvStrict(value: string | undefined, name = "SANDBOX_BACKEND"): Config["sandboxBackend"] {
  ...
  if (
    ...
    return backend;
  throw new Error(
    `${name}=${JSON.stringify(value)} is not recognized — use local, smolmachines, e2b, modal, or agent37, or unset it.`,
  );
}
...
export function loadConfig(env: NodeJS.ProcessEnv = process.env): Config {
  ...
  if (env.NODE_ENV === "production" && !env.SANDBOX_BACKEND?.trim()) {
    throw new Error(
      "SANDBOX_BACKEND must be set explicitly in production — use smolmachines, e2b, modal, agent37, or local.",
    );
  }
  ...
  return {
    ...
    ...(env.DEPLOY_GATE_SECRET ? { deployGateSecret: env.DEPLOY_GATE_SECRET } : {}),
  };
}
