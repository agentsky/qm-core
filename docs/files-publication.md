...
The core's object-store credentials need `s3:GetObject`, `s3:PutObject`, `s3:AbortMultipartUpload`, and `s3:ListMultipartUploadParts` on its Files prefix, plus `s3:ListBucket` and `s3:ListBucketMultipartUploads` on the bucket. Buckets with server-side encryption also need the applicable key permissions to complete uploads and retrieve checksums. The sandbox needs outbound HTTPS to the S3 endpoint but receives no object-store credentials.
